...
// How often to run heavy inference (ms).
// Built-in + skin run every frame (they're fast).
// CDN modes are throttled — phone CPUs can't keep up every frame.
const CDN_INTERVAL_MS = 300; // ~3 fps inference, video still renders at full fps
let lastInferenceTime = 0;
let lastDrawnItems = []; // keep last detections painted while waiting
...
async function loop() {
  ...
  if (video.readyState >= 2) {
    ...
    const isCdnMode = currentMode === 'faceapi' || currentMode === 'full';

    // For CDN modes: repaint last known boxes every frame so video feels smooth,
    // but only re-run inference at CDN_INTERVAL_MS rate.
    if (isCdnMode && (now - lastInferenceTime) < CDN_INTERVAL_MS) {
      // Just redraw last detections on fresh frame — no inference
      ctx.clearRect(0, 0, W, H);
      lastDrawnItems.forEach(item => {
        if (item._box) {
          const { x, y, w, h, color, label, conf } = item._box;
          drawBox(x, y, w, h, color, label, conf);
        }
      });
      animId = requestAnimationFrame(loop);
      return;
    }
    if (isCdnMode) lastInferenceTime = now;
      ...
      try {
        ...
        resized.forEach(det => {
          ...
          allItems.push({ label: 'Face', cat: 'Face', color: '#00ffe0', conf,
            _box: { x: mx, y, w: width, h: height, color: '#00ffe0', label: 'FACE', conf } });
        });
        const n = resized.length;
        if (n > 0) sendPhoneAlert(n);
        countEl.innerHTML = n > 0 ? `${n}<span>${n === 1 ? 'face detected' : 'faces detected'}</span>` : '';
        lastDrawnItems = allItems;
      } catch(e) {
        dset(dErr, e.message.slice(0, 80), '');
